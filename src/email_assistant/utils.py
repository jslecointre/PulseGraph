...
def format_gmail_markdown(subject, author, to, email_thread, email_id=None):
    ...
    if email_thread and (
        email_thread.strip().startswith("<!DOCTYPE") or email_thread.strip().startswith("<html") or "<body" in email_thread
    ):
...
def show_graph(graph, output_file_path: str, xray=False):
    ...
    try:
        ...
        return Image(graph.get_graph(xray=xray).draw_mermaid_png(output_file_path=output_file_path))
        ...
        return Image(
            graph.get_graph().draw_mermaid_png(draw_method=MermaidDrawMethod.PYPPETEER, output_file_path=output_file_path)
        )


def str_to_bool(env_value: str) -> bool:
    ...
    if val in false_values:
        return False
    raise ValueError(f"Invalid boolean string: {env_value}")
