...
def fetch_group_emails(
    ...
    if not use_mock and not gmail_token and not gmail_secret:
        ...
        if not os.path.exists(token_path) and not os.path.exists(secrets_path):
            logger.warning("No Gmail API credentials found. Looking for token.json or secrets.json in .secrets directory")
...
class SendEmailInput(BaseModel):
    ...
    email_id: str = Field(
        description="Gmail message ID to reply to. This must be a valid Gmail message ID obtained from the fetch_emails_tool. If you're creating a new email (not replying), you can use any string like 'NEW_EMAIL'."  # noqa: E501
    )
...
def send_email(email_id: str, response_text: str, email_address: str, addn_recipients: Optional[List[str]] = None) -> bool:
    """
    ...
    Args:
        ...
        addn_recipients: Optional additional recipients
    ...
    try:
        ...
        try:
            ...
            logger.info(f"Retrieved message ID {email_id} : [{message}]")
...
def send_email_tool(
    email_id: str, response_text: str, email_address: str, additional_recipients: Optional[List[str]] = None
) -> str:
    """
    ...
    """
    try:
        logger.info(f"[TOOL] GMAIL Sending send_email_tool ID : [{email_id}] from [{email_address}] content [{response_text}]")
        success = send_email(email_id, response_text, email_address, addn_recipients=additional_recipients)
        if success:
    ...
    try:
        ...
        for date_str in dates:
            ...
            for event in events:
                ...
                start_tz = ZoneInfo(event["start"].get("timeZone", tz))

                end = event["end"].get("dateTime", event["end"].get("date"))
                end_tz = ZoneInfo(event["start"].get("timeZone", tz))

                # Convert to datetime objects
                if "T" in start:  # dateTime format
                    start_dt = datetime.fromisoformat(start.replace("Z", "+00:00")).astimezone(start_tz)
                    end_dt = datetime.fromisoformat(end.replace("Z", "+00:00")).astimezone(end_tz)
            ...
            if "all-day" in [slot[0] for slot in busy_slots]:
                ...
                work_start = datetime(year=int(year), month=int(month), day=int(day), hour=9, minute=0, tzinfo=tz)
                work_end = datetime(year=int(year), month=int(month), day=int(day), hour=17, minute=0, tzinfo=tz)

                # convert work window to UTC so all comparisons are in UTC
                ...
                for start, end in busy_slots:
                    if current < start:
                        available_slots.append((current, min(start, work_end)))
...
class ScheduleMeetingInput(BaseModel):
    ...
    timezone: str = Field(default=tz_str, description="Timezone for the meeting")


def send_calendar_invite(
    attendees: List[str], title: str, start_time: str, end_time: str, organizer_email: str, timezone: str = "America/Toronto"
) -> bool:
    """
...
def schedule_meeting_tool(
    attendees: List[str], title: str, start_time: str, end_time: str, organizer_email: str, timezone: str = tz_str
) -> str:
    """
    Schedule a meeting with Google Calendar and send invites.
    ...
    Args:
        ...
        end_time: Meeting end time in ISO format (YYYY-MM-DDTHH:MM:SS)
        organizer_email: Email address of the meeting organizer
        timezone: Timezone for the meeting (default: America/Toronto)
...
if __name__ == "__main__":
    ...
    logging.basicConfig(level=logging.DEBUG, handlers=logger.handlers, force=True)
    print("Invoke tool")
    # get_calendar_events_args = {"dates": ["12-08-2025"]}
    # observation = check_calendar_tool.invoke(input=get_calendar_events_args)
    # print(observation)
    schedule_meeting_args = {
        "attendees": ["my-name@email.com", "agentic@gmail.com"],
        "title": "Tax Planning Strategies Discussion",
        "start_time": "2025-08-12T14:00:00",
        "end_time": "2025-08-12T14:30:00",
        "organizer_email": "agentic@gmail.com",
        "timezone": tz_str,
    }

    observation = schedule_meeting_tool.invoke(input=schedule_meeting_args)
    print(observation)
